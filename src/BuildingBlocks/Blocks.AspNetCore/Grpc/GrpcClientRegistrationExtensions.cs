...
public static class GrpcClientRegistrationExtensions
{
        ...
        where TClient : class
    {
        ...
        if (string.IsNullOrEmpty(serviceKey) || !grpcOptions.Services.TryGetValue(serviceKey, out var serviceSettings))
            throw new InvalidOperationException($"Missing Grpc service config for: {typeof(TClient).Name} ");
